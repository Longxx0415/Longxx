...
def main():
    ...
    for combo_idx, combo in enumerate(COMBOS):
        ...
        if len(gpr_idx_26) > 0:
            ...
            # 保存超参数
            params_26self[combo_name] = extract_gpr_params(gpr)
        else:
    ...
    for row_name, hp_key in hp_map.items():
        ...
        for i, (combo_name, _) in enumerate(best3_mode2):
            safe_name = combo_name.replace('+', '_')
            val = params_26self[combo_name].get(hp_key, '')
            if isinstance(val, float):
                val = round(val, 6)
            for suffix in [f'{safe_name}_预测', f'{safe_name}_vs参数1差值', f'{safe_name}_vs参数2差值']:
                row[suffix] = val if '预测' in suffix else ''
    ...
    ax.set_title('模式二（26→26）最优3个组合预测 vs 官方参数1', fontweight='bold')
    ax.set_xticks(x)
    ...
    plt.suptitle('GPR 模式二（26→26）最优3个组合分析',
