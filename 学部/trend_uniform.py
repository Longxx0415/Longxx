...
def build_q1_model(df_2026):
    ...
    def compute_H(df):
        ...
        scores = input1_new @ components.T
        H = scores @ var_ratio
        return H
...
def q2_trend_predict(history_dfs, target_year_offset, dept_idx, criterion='bic'):
    """
    Q2趋势回归：用年份的一次项和二次项做最优子集选择
    有效组合：{year_idx} 或 {year_idx, year_idx_sq}
    有二次项时必须同时包含一次项
    """
    ...
    valid_combos = [
        ['year_idx'],
        ['year_idx', 'year_idx_sq'],
    ]

    for combo in valid_combos:
        X_dict = {'const': np.ones(n)}
        if 'year_idx' in combo:
            X_dict['year_idx'] = x
        if 'year_idx_sq' in combo:
            X_dict['year_idx_sq'] = x**2
    ...
    if best_model is None:
        if n >= 2:
            slope = (y[-1] - y[0]) / (n - 1)
            return float(y[-1] + slope * target_year_offset)
        return float(y[-1])

    target_idx = (n - 1) + target_year_offset
    pred_dict = {'const': 1.0}
    if 'year_idx' in best_features:
        pred_dict['year_idx'] = target_idx
    if 'year_idx_sq' in best_features:
        pred_dict['year_idx_sq'] = target_idx ** 2
...
print(f"Q2方法: 趋势回归（年份一次项+二次项，BIC选优）")
print(f"组合定权: 滚动窗口验证 → 各学院独立权重 → 取平均得统一权重")
...
for w_idx, (desc, hist_dfs, actual_df, offset) in enumerate(windows):
    ...
    q2_pred = np.array([q2_trend_predict(hist_dfs, offset, i) for i in range(n_depts)])
...
# 各学院独立权重
w_q1_dept = (1 / var_q1) / (1 / var_q1 + 1 / var_q2)
w_q2_dept = (1 / var_q2) / (1 / var_q1 + 1 / var_q2)

# 【关键改动】取权重平均值，得到统一权重
w_q1_global = np.mean(w_q1_dept)
w_q2_global = np.mean(w_q2_dept)

print(f"\n--- 各学院独立权重（方差倒数法）---")
for i, name in enumerate(dept_names):
    print(f"    {name}: w_Q1={w_q1_dept[i]:.4f}, w_Q2={w_q2_dept[i]:.4f}")

print(f"\n--- 【统一权重】取所有学院权重平均值 ---")
print(f"    全局 w_Q1 = {w_q1_global:.4f}")
print(f"    全局 w_Q2 = {w_q2_global:.4f}")
print(f"    验证: w_Q1 + w_Q2 = {w_q1_global + w_q2_global:.4f}")

# =================== 假设检验：用22-23-24-25预测26年 ===================
print(f"\n{'='*70}")
print(">>> 假设检验与预测精度（验证26年，未约束，统一权重）")
print(f"{'='*70}")

q1_26 = q1_predict(df_2026)
q2_26 = np.array([q2_trend_predict([dfs[2022], dfs[2023], dfs[2024], dfs[2025]], 1, i)
                  for i in range(n_depts)])

# 【统一权重】组合预测
qc_26 = w_q1_global * q1_26 + w_q2_global * q2_26
qc_26 = np.maximum(qc_26, 0)
...
# =================== 最终预测：30年 ===================
print(f"\n{'='*70}")
print(">>> 最终预测：2030年编制人数（统一权重）")
print(f"{'='*70}")

q1_30 = q1_predict(df_forecast)
q2_30 = np.array([q2_trend_predict([dfs[2022], dfs[2023], dfs[2024], dfs[2025], dfs[2026]], 4, i)
                  for i in range(n_depts)])

# 【统一权重】组合预测
qc_30 = w_q1_global * q1_30 + w_q2_global * q2_30
...
save_path_metrics = safe_save(test_summary, 'result_2030_metrics_trend_uniform.csv')
save_path_26 = safe_save(result_26, 'result_2026_validation_trend_uniform.csv')
save_path_30 = safe_save(result_30, 'result_2030_forecast_trend_uniform.csv')
